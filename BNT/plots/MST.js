...
function createMST(choosen_links, groups_obj, graph, nodes, links, max_value, min_value) {
    ...
    var factor = d3.scale.linear().domain([min_value, max_value]).range([20, 60]);
...
function filteringProcess(data, spearman, kruskal, chi, min_pvalue){
    ...
    for(var i = 0; i < chi.length; i++){
        ...
        obj.source = chi[i].x;
        obj.target = chi[i].y;
        obj.value = 1 - Math.pow(Number(chi[i].cramer_v),2);
        obj.effect_size = Math.pow(Number(chi[i].cramer_v),2);
